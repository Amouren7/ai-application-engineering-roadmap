...
假设用户说：“帮我退订单 A100。”系统不要马上退款，而是产生下面的状态：

| 步骤 | 状态字段 | 允许的下一步 |
| --- | --- | --- |
| 提取订单号 | `order_id=A100` | 查询订单 |
| 查询订单 | `status=delivered` | 检查规则 |
| 检查规则 | `eligible=true, amount=99` | 展示预览 |
| 等待确认 | `confirmed=false` | 等用户回答 |
| 用户确认 | `confirmed=true` | 执行退款 |
| 执行完成 | `refund_status=success` | 输出结果 |

每一步只修改自己负责的字段，并把结果写进日志。这样出错时能看见“查到了订单，但还没有确认”，而不是只得到一个模糊的失败。

## 工具参数的双重校验

第一层是 schema，检查 `order_id` 是否为字符串；第二层是业务校验，检查订单是否属于当前用户、金额是否在允许范围。格式正确不代表有权执行。

```text
schema 校验：order_id 有值吗？
身份校验：这个订单属于当前用户吗？
状态校验：订单允许退款吗？
金额校验：退款金额是否超过上限？
确认校验：用户是否看过预览并明确同意？
...
如果用户在确认页面停留十分钟，系统重启后仍然应该知道订单、金额和当前阶段；但不能因为恢复状态就自动执行。恢复后仍要重新确认敏感动作。

## 本节练习

把“预约会议室”画成 Workflow：检查时间 → 查询空闲 → 展示地点和费用 → 用户确认 → 创建预约。为每一步写成功、失败和取消三种结果。
